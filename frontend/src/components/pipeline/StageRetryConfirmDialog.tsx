...
interface StageRetryConfirmDialogProps {
  open: boolean
...
export function StageRetryConfirmDialog({
  open,
  onOpenChange,
  onConfirm,
  stageName,
  isLoading = false,
}: StageRetryConfirmDialogProps) {
  return (
      ...
      <Dialog.Portal>
        ...
        >
          <Dialog.Title className="text-[24px] font-semibold text-[var(--color-foreground)] mb-2">
            Retry {stageName} stage?
          </Dialog.Title>
          <Dialog.Description className="text-[16px] font-normal text-[var(--color-muted)] mb-6">
            Failed records in this stage will be requeued for processing.
          </Dialog.Description>
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            >
              {isLoading ? 'Retrying…' : 'Retry Stage'}
            </button>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
