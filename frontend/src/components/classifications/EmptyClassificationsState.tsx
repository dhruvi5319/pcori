...
export function EmptyClassificationsState({
  hasActiveFilters,
  onUpload,
  onClearFilters,
}: EmptyClassificationsStateProps) {
  if (hasActiveFilters) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        {onClearFilters && (
          <button
            ...
            className="text-[#1D4ED8] dark:text-[#3B82F6] underline text-[16px] hover:opacity-80 transition-opacity"
          >
            Clear all filters
          </button>
        )}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-4">
      <FileSearch className="w-12 h-12 text-gray-400" aria-hidden="true" />
      <h3 className="text-[24px] font-semibold text-gray-900 dark:text-white">
        No research plans yet
      </h3>
      <p className="text-[16px] text-gray-500 text-center max-w-md">
        Upload your first research plan PDF to get started. The system will automatically classify
        it against the PCORI taxonomy.
      </p>
      {onUpload && (
        <button
          onClick={onUpload}
