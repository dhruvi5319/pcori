...
| JTBD-01.4 | In-app alert on NEEDS_REVIEW | **Given** a classification transitions to `NEEDS_REVIEW`, **when** the reviewer is logged into the application, **then** the notification bell shows an unread badge within the next polling cycle (≤10 seconds) |
